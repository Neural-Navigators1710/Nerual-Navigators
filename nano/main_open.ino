...
unsigned long startupIgnoreEnd = 0;   // NEW: Ignore GAP detection at startup
...
char lastObstacle = 'N'; // 'L' = left, 'R' = right, 'N' = none
...
// === TURN COUNT ===
// ===================================================
int turnCount = 0;  // count turns from TOF gaps

// ===================================================
// === SPEED CONTROL ===
// ===================================================
const int STRAIGHT_SPEED = 110;  // Normal driving speed
const int TURN_SPEED = 90;      // Reduced speed while turning
...
void setup() {
  ...
  // Ignore GAP detection for first 3 seconds
...
void handleTOF(float currentYaw) {
  ...
  // --- GAP detection (only if not turning and after startup delay) ---
  if (!turning && millis() > gapCooldownEnd && millis() > startupIgnoreEnd) {
